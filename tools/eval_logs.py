...
num_starts_per_dataset = 20 # number of trials per dataset
...
def get_ratio(alg, get_mean_error=False, prefix=""):
    ...
    for dataset_index, dataset_name in enumerate(datasets):
        ...
        for start_num, gt_file in enumerate(gt_files):
            ...
            est_file = log_directory + "/" + alg + "_" + prefix + "use_annealing_" + dataset_name + "_" + str(img_num) + "_" + "poses.npy"
            pose_estimates = np.load(est_file)

            if alg == 'mocnerf':
                iteration_file = log_directory + "/" + alg + "_" + prefix +  "use_annealing_" + dataset_name + "_" + str(img_num) + "_" + "forward_passes.npy"
...
#plt.plot(mcl_forward_passes_count, mcl_average_position_metric, label="Loc-Splats mit Annealing", linewidth=2.5, color='b')
...
plt.xlabel("Anzahl der gerenderten Partikel", fontsize=fontsize, family='Arial')
plt.grid()
plt.legend(loc='upper left', fontsize=fontsize-4)

if not get_mean_error:
    plt.ylim([0, 1.01])
    plt.title("Genauigkeitsevaluation der geschätzten Position", fontsize=fontsize, family='Arial')
    plt.ylabel("Anteil der Versuche mit Positionsfehler \n Error < 10 cm", fontsize=fontsize, family='Arial')
else:
    plt.title("Durchschnittlicher Positionsfehler", fontsize=fontsize, family='Arial')
...
#plt.plot(mcl_forward_passes_count, mcl_average_rotation_metric, label="LocSplats mit Annealing", linewidth=2.5, color='b')

plt.plot(mcl_forward_passes_count, mcl_average_rotation_metric, label="Versuche mit Rotationsfehler < 10$^\circ$", linewidth=2.5, color='b')
plt.plot(mcl_forward_passes_count, mcl_average_position_metric, label="Versuche mit Positionsfehler < 10 cm", linewidth=2.5, color='orange')
...
if not get_mean_error:
    plt.ylim([0, 1.01])
    plt.title("Evaluation der Rotationsgenauigkeit", fontsize=fontsize, family='Arial')
    plt.ylabel("Anteil der Versuche", fontsize=fontsize, family='Arial')
else:
    plt.title("Durchschnittlicher Rotationsfehler", fontsize=fontsize, family='Arial')
    plt.ylabel("error (Grad)", fontsize=fontsize, family='Arial')
plt.tight_layout()
plt.show()
